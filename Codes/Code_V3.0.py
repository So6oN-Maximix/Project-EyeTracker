...
def display_graph(size,X,Y,S,C,colorbar_label,title,xlabel,ylabel,xlimit=None,ylimit=None,image=None,not_show=None, arrow=None):
    ...
    if not not_show:
        ...
        print(f"Image enregistrée : {Reduction_video.PROTECH_PATH/'Outputs'/'Graphs'/file_name}")
    if not arrow:
        plt.close()
...
def exporter_donnees_clusters_avec_ordre(pos_x, pos_y, counts, fixation_order, filename="Clusters_Fixation_Ordre.csv"):
...
def display_graph_arrow(size,X,Y,S,C,colorbar_label,title,xlabel,ylabel,hwidth,hlength,xlimit=None,ylimit=None):
    display_graph(size,X,Y,S,C,colorbar_label,title,xlabel,ylabel,not_show=True,arrow=True)
    ...
    for i, (current_x, current_y) in enumerate(zip(X,Y)):
        if previous_position is not None:
            plt.arrow(previous_position[0], previous_position[1], current_x - previous_position[0], current_y - previous_position[1], head_width=hwidth, head_length=hlength, fc='blue', ec='blue', length_includes_head=True)
...
def clusters_fixations(size,X,Y,C,title1,title2,width=None,height=None):
    ...
    display_graph_arrow(size, X, Y, 800, C, "Nombre de Fixations",title1 + " - échelle video", "Gaze2dX(pixel)","Gaze2dY(pixel)",0.005,0.01, (0, width), (0, height))
    display_graph_arrow(size, X, Y, 200, C, "Nombre de Fixations",title1 + " - échelle étendue", "Gaze2dX(pixel)","Gaze2dY(pixel)",0.005,0.01, (-width, 2 * width), (-height, 2 * height))
    ...
    if last_frame is not None:
        ...
        display_graph_arrow(size, X, adjusted_pos_y, 800, C,"Nombre de Fixations",title1 + " - échelle video","Gaze2dX(pixel)", "Gaze2dY(pixel)", 0.005, 0.01, (0, width), (0, height))
...
video_name,csv_file,input_timecode01,input_timecode02,csv_file_timeline=Console_execution.main()

Test_timeline.creation_timeline(csv_file_timeline)

new_csv,new_video=Reduction_video.reduction(video_name,csv_file,input_timecode01,input_timecode02)
...
display_graph((8,6),pos_x,pos_y,200,counts,"Nombre de Fixations","Clusters de Fixation  - échelle étendue","Gaze2dX(pixel)","Gaze2dY(pixel)",(-1 * width, 2 * width),(-1 * height, 2 * height))
...
if last_frame is not None:
    ...
    display_graph((10, 8), pos_x, adjusted_pos_y, 800, counts, "Nombre de Fixations","Superposition de la Dernière Frame avec les Clusters - echelle video", "Gaze2dX(pixel)", "Gaze2dY(pixel)",(0, width),(0, height),1)
    display_graph((10, 8), pos_x, adjusted_pos_y, 200,counts,"Nombre de Fixations","Superposition de la Dernière Frame avec les Clusters - echelle étendue","Gaze2dX(pixel)","Gaze2dY(pixel)",(-1 * width, 2 * width),(-1 * height, 2 * height),1)
    display_graph((10,8),pos_x, adjusted_pos_y,800,counts,"Nombre de Fixations","Clusters de Fixation ajustés à l'échelle de l'image - echelle video","Gaze2dX(pixel)","Gaze2dY(pixel)",(0, width),(0, height))
    display_graph((10,8),pos_x, adjusted_pos_y,200,counts,"Nombre de Fixations","Clusters de Fixation ajustés à l'échelle de l'image - echelle étendue","Gaze2dX(pixel)","Gaze2dY(pixel)",(-width, 2*width),(-height, 2*height))
else:
    print("Impossible d'extraire la dernière frame de la vidéo.")
...
if last_frame is not None:
    ...
    display_graph((10, 8), pos_x, adjusted_pos_y, 800, times, "Temps de fixation cumulé (ms)","Superposition de la Dernière Frame avec les Clusters 01", "Gaze2dX(pixel)", "Gaze2dY(pixel)",(0, width),(0, height),1)
    display_graph((10, 8), pos_x, adjusted_pos_y, 200, times, "Temps de fixation cumulé (ms)","Superposition de la Dernière Frame avec les Clusters 02", "Gaze2dX(pixel)", "Gaze2dY(pixel)",(-1*width, 2*width),(-height, 2*height),1)
    display_graph((10, 8), pos_x, adjusted_pos_y, 800, times, "Temps de fixation cumulé (ms)","Temps de Fixation cumulé par Cluster - échelle video 01", "Gaze2dX(pixel)", "Gaze2dY(pixel)",(0, width),(0, height))
    display_graph((10, 8), pos_x, adjusted_pos_y, 200, times, "Temps de fixation cumulé (ms)","Temps de Fixation cumulé par Cluster - échelle video 01", "Gaze2dX(pixel)", "Gaze2dY(pixel)",(-width, 2*width),(-height, 2*height))
else:
    print("Impossible d'extraire la dernière frame de la vidéo.")
...
positions = np.array([[float(gaze2dx[i]), float(gaze2dy[i])] for i in fixations_idx if gaze2dx[i] not in [None, 'NA'] and gaze2dy[i] not in [None, 'NA']])
...
### Affichage graphs
Carte_de_chaleur.display_cartes_chaleur(t0, fixation_time_filtered, positions, width, height, Reduction_video.PROTECH_PATH/"Outputs"/new_video, media_frame_indices, media_time_stamp)
Carte_de_chaleur.display_cartes_chaleur(tmilieu, fixation_time_filtered, positions, width, height, Reduction_video.PROTECH_PATH/"Outputs"/new_video, media_frame_indices, media_time_stamp)
Carte_de_chaleur.display_cartes_chaleur(tfinal, fixation_time_filtered, positions, width, height, Reduction_video.PROTECH_PATH/"Outputs"/new_video, media_frame_indices, media_time_stamp)
Carte_de_chaleur.display_cartes_chaleur(random_instant, fixation_time_filtered, positions, width, height, Reduction_video.PROTECH_PATH/"Outputs"/new_video, media_frame_indices, media_time_stamp)
...
Carte_de_chaleur.afficher_carte_chaleur_avec_derniere_frame_valide(tfinal, Reduction_video.PROTECH_PATH/"Outputs"/new_video, media_frame_indices, media_time_stamp, positions, fixation_time_filtered)
...
extraire_donnees_utiles_excel("Donnees_cartes_de_chaleur.csv",[t0, tmilieu,random_instant, tfinal],media_frame_indices,media_time_stamp,positions)
...
clusters_fixations((10, 8), filtered_pos_x, filtered_pos_y,filtered_counts,f"Clusters de Fixation (sup {seuil} positions) avec Flèches pour le Chemin",f"Superposition de la Dernière Frame avec les Clusters (sup {seuil} positions)",width,height)
