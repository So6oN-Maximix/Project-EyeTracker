...
def lecture(file_name):
    ...
    for i in range(nb_datas):
        for j in range(3, len(datas)):
            if datas[j][2 * i + 1] != "" or datas[j][2 * i + 2] != "":
                debut, fin = convertion_secounds(
                    datas[j][2 * i + 1]
                ), convertion_secounds(datas[j][2 * i + 2])
                if j < index:
    ...
    for i in range(3, len(datas)):
        if datas[i][2 * nb_datas + 1] != "":
            if i < index:
                segments[nb_datas].append(
                    convertion_secounds(datas[i][2 * nb_datas + 1])
                )
            else:
                segments[2 * (nb_datas + 1) - 1].append(
                    convertion_secounds(datas[i][2 * nb_datas + 1])
                )
...
def creation_timeline(file_name):
    ...
    segments = (
        all_segments[:nbr_datas] + all_segments[nbr_datas + 1 : len(all_segments) - 1]
    )
    ...
    for i in range(len(names)):
        ...
        ax.tick_params(
            axis="x", rotation=50, labelsize=9
        )  # Rotation des timecodes de 90°
        ...
        color_list = [
            ...
            "#B22222",
            "#4682B4",
        ]  # Violet, Turquoise, Corail, Ocre, Rouge brique, Bleu acier
        assert len(color_list) >= len(
            segments
        )  # Pour avoir assez de couleur à dessiner
        ax.broken_barh(
            segments_gris, (y_position, bar_height), facecolors="#D3D3D3"
        )  # Gris
        for j in range(len(segments) // 2):
            if segments[2 * i + j] != []:
                ax.broken_barh(
                    ...
                    facecolors=color_list[j],
                )

        for annot in annotation:
            ax.annotate(
                "Mise en forme",
                xy=(annot, y_position + bar_height),
                xytext=(annot, y_position + bar_height + 3),
        ...
        plt.close()
        print(
            f"Image enregistrée : {PROTECH_PATH/'Outputs'/f'Timeline_{names[i]}.png'}"
        )
    print("================ FIN TIMELINES ================\n")
