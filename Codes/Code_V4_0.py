...
def display_graph(
    ...
    """
    C'est le moteur principal de la sauvegarde des graphs au cours du programme

    Args :\n
        size (int,int) : Taille souhaitée du graph\n
        X, Y (listes) : Listes des données à tracer sur le graph\n
        S, C (listes) : Données utiles pour la gestion de taille et de carte de chaleur\n
        colorbar_label, title, xlabel, ylabel (str) : Titre des différentes parties du graphs\n
        xlimit, ylimit (float) : Si donnés en argument, sert à délimiter le graph\n
        bg_image (array): Si donné en argument, affiche en arrière plan une image\n
        arrow (Booleen) : Permet de dire si l'on affiche les clusters avec des fleches ou non\n
        arrow_params ({str:int}) : Dictionnaire qui contient des paramètres graphiques pour l'affichage avec flèches
    """
...
def extraire_donnees_utiles_excel(file_name, instants_cibles, df_full):
    """
    Permet de creer un fichier csv pour stocker les données calculées

    Args :\n
        file_name (str) : Nom du fichier de sauvegarde\n
        instants_cibles (liste) : Liste des instants que l'on veut sauvegarder dans le fichier de sortie\n
        df_full (pandas) : Tableau CSV entier mais gérer par la bibliothèque pandas (sert à récupérer les données associèes à instants_cibles)
    """
    output_data = {
...
def save_dataframe_as_transposed_csv(filename, data_dict):
    """
    Utilise la fonction sauvegarde de Reduction_video.py afin d'enregistrer les données fournies en argument

    Args :\n
        filename (str) : Nom du fichier de sauvegarde\n
        data_dict ({str : liste}) : Dictionnaire qui contient les données à sauvegarder avec les noms de colonnes
    """
...
def clusters_fixations_plot_suite(
    ...
    """
    Effectue les différents affichages des graphs correspondant au clusters de fixations

    Args :\n
        X, Y (listes) : Données à tracer sur le graph\n
        C : Donnée utilie à la gestion des cartes de chaleurs\n
        title_base, title_superpo : Titres des graphs sans et avec image de fond\n
        width, height (int) : Données servant à la gestion de la taille du graph\n
        last_frame (cv2) : Correspond à l'image qui sera en arrière (si n'est pas None)
    """
    size_std = (8, 6)
    size_large = (10, 8)
...
def main():
    """
    Fonction d'éxecution, sert pour eviter les conflits avec pdoc lors de la création de la documentation (à cause de l'input dans Console_execution.py
    """
